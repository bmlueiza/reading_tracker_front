<template>
    <div class="home_reading">
      ...
      <div v-if="books.length === 0">¡Aún no has comenzado a leer un libro!</div>
      ...
      <button v-if="books.length > 5" @click="viewAll('reading')">Ver todos</button>
    </div>
</template>
...
<script setup>
  ...
  onMounted(async () => {
    try {
      ...
      console.error("Error al obtener libros en lectura actual", error);
    }
  });
...
<style scoped src="../styles/components/reading-books.css"></style>  
